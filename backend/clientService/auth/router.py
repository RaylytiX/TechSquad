...
@router.post("/login")
async def login(response: Response, user_data: UserAuth, background_tasks: BackgroundTasks, db: db_dependency = db_dependency):
...
async def signup(info_user: UserAuth, background_tasks: BackgroundTasks, db: db_dependency = db_dependency):
    ...
    response = JSONResponse(
        content=f"Вы успешно зарегистрированы {db_user.email}!",
...
@router.post("/logout")
async def logout_user(response: Response, background_tasks: BackgroundTasks, user: UserBase=Depends(get_current_user), db: db_dependency = db_dependency):
